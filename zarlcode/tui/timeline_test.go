...
func TestToolItemRunningStateHasTextLabel(t *testing.T) {
	out := drive(t, teasink.ToolStartedMsg{TaskID: "t1", ToolID: "c1", ToolName: "grep"})
...
func TestTimeline_LoadedSkillAndAgentStayInNestedToolGroup(t *testing.T) {
	out := drive(t,
		...
		teasink.ConversationStartedMsg{TaskID: "child1", Depth: 1, ParentToolCallID: "a1", AgentName: "go-code-reviewer", Prompt: "review"},
	...
	for _, want := range []string{"use helpers", "tools (2)", "[-] agents (1)", "go-code-reviewer: review", "[+] skills (1): go-testing"} {
		if !strings.Contains(out, want) {
			t.Errorf("timeline missing %q in:\n%s", want, out)
		}
	}
