...
func TestGuardrailSummary_DerivesFanoutValue(t *testing.T) {
	...
	if got := tui.GuardrailSummary(deps); !strings.Contains(got, "grep≤99") {
...
func TestGuardrailSummary_EmptySentinel(t *testing.T) {
	if got := tui.GuardrailSummary(guardrails.Deps{}); got != "(none configured)" {
