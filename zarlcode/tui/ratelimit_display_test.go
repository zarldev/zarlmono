...
func TestFormatRateLimit(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := tui.FormatRateLimit(tc.in); got != tc.want {
				t.Errorf("tui.FormatRateLimit() = %q, want %q", got, tc.want)
			}
		})
	}
}
...
func TestFormatRateLimit_SoonReset(t *testing.T) {
	...
	if got := tui.FormatRateLimit(e); got != want {
		t.Errorf("tui.FormatRateLimit() = %q, want %q", got, want)
	}
}
