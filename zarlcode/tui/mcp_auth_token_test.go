...
func TestResolveMCPAuthToken_MigratesLegacyPlaintext(t *testing.T) {
	...
	if got := tui.ResolveMCPAuthToken(t.Context(), s, rows[0]); got != secret {
	...
	if k, err := s.Svc.GetKey(t.Context(), prefs.ScopeGlobal, tui.MCPAuthKeyProvider(name)); err != nil || k != secret {
...
func TestResolveMCPAuthToken_NoVaultFallsBackToPlaintext(t *testing.T) {
	...
	if got := tui.ResolveMCPAuthToken(t.Context(), s, row); got != "plain" {
