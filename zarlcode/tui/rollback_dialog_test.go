...
// stepUI applies a Bubble Tea message while preserving the caller's UI handle.
func stepUI(t testing.TB, m *tui.UI, msg tea.Msg) {
	t.Helper()
	got, _ := m.Update(msg)
	if got != m {
		t.Fatalf("Update returned %T, want *UI", got)
	}
}
