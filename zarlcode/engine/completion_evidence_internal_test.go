...
func TestCompletionEvidenceTracksMutationAndVerificationOrder(t *testing.T) {
	...
	src := newEvidenceSource(evidenceFakeSource{results: map[tools.ToolName]*tools.ToolResult{
...
func TestEvidenceAwareCompletionCorrectsOnceInsideRunner(t *testing.T) {
	t.Parallel()
	evidence := newCompletionEvidence()
	source := newEvidenceSource(tools.NewRegistry(evidenceMutationTool{}), evidence)
	quality := newPlanAwareTurnQuality(nil, func() bool { return false }, evidence)
	client := runnertest.NewClient([][]llm.CompletionChunk{
		{runnertest.ChunkToolCall("edit-1", "edit", `{}`), runnertest.ChunkDone()},
		{runnertest.ChunkText("done"), runnertest.ChunkDone()},
		{runnertest.ChunkText("verified manually"), runnertest.ChunkDone()},
	})
	r := runner.New(client,
	...
	for _, message := range result.Messages {
		if message.Role == llm.RoleUser && message.Content == verifyAfterChangeCorrection {
