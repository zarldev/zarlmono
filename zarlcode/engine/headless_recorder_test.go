...
func TestHeadlessRecorder_PersistsLifecycle(t *testing.T) {
	...
	ctx := t.Context()
	rec := engine.NewHeadlessRecorder(store, "run-1", t.TempDir())
...
func TestHeadlessRecorder_PersistsAttempt(t *testing.T) {
	...
	ctx := t.Context()
	rec := engine.NewHeadlessRecorder(store, "run-attempt", t.TempDir())
	rec.Start(ctx, "fix the bug", "llamacpp", "qwen3.6")
...
func TestHeadlessRecorder_PersistsVerifierResult(t *testing.T) {
	...
	ctx := t.Context()
	rec := engine.NewHeadlessRecorder(store, "run-verifier", t.TempDir())
	rec.Start(ctx, "fix the bug", "llamacpp", "qwen3.6")
	code := 7
...
func TestHeadlessRecorder_NilIsNoOp(t *testing.T) {
	var rec *engine.HeadlessRecorder // no store configured
