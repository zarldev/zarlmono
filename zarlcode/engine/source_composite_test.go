...
type sourceFakeTool struct{ name tools.ToolName }

func (t sourceFakeTool) Definition() tools.ToolSpec { return tools.ToolSpec{Name: t.name} }

func (sourceFakeTool) Execute(context.Context, tools.ToolCall) (*tools.ToolResult, error) {
	return &tools.ToolResult{}, nil
}

func TestCompositeSource_SeesSecondaryRegistrationsAfterConstruction(t *testing.T) {
	primary := tools.NewRegistry()
	primary.Register(sourceFakeTool{name: code.ToolNameRead})
	secondary := tools.NewRegistry()
	src := engine.ComposeSources(primary, secondary)

	if sourceHasRemoteTool(t.Context(), src) {
		t.Fatal("remote_tool should not be visible before registration")
	}

	secondary.Register(sourceFakeTool{name: "remote_tool"})
	if !sourceHasRemoteTool(t.Context(), src) {
...
func TestCompositeSource_PrimaryWinsOnDuplicateNames(t *testing.T) {
	...
	primary.Register(sourceFakeTool{name: code.ToolNameRead})
	secondary := tools.NewRegistry()
	secondary.Register(sourceFakeTool{name: code.ToolNameRead})
...
func (s *countingSource) Tools(context.Context) iter.Seq[tools.Tool] {
	...
	return func(yield func(tools.Tool) bool) {
		for _, n := range s.names {
			if !yield(sourceFakeTool{name: n}) {
				return
			}
		}
	}
