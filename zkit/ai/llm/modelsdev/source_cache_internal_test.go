...
type countingSnapshotCache struct {
	inner cache.Cache[string, Snapshot]
	gets  atomic.Int64
}
...
func TestSourceStaleSnapshotBacksOffAfterRefreshFailure(t *testing.T) {
	...
	stale := Snapshot{
		Schema: snapshotSchema, FetchedAt: time.Now().Add(-time.Hour),
		Entries: map[string]map[string]Entry{"openai": {"gpt-test": {}}},
	}
	if err := store.Set(t.Context(), snapshotKey, stale); err != nil {
		t.Fatal(err)
	}
	source := New(store, WithTTL(time.Minute), WithBaseURL(server.URL))
	for range 5 {
...
func (c *countingSnapshotCache) Get(ctx context.Context, key string) (Snapshot, error) {
	c.gets.Add(1)
	return c.inner.Get(ctx, key)
}

func (c *countingSnapshotCache) Set(ctx context.Context, key string, value Snapshot) error {
...
func (c *countingSnapshotCache) Healthy() error { return c.inner.Healthy() }
...
func TestSourceLookupCachesSnapshotInMemory(t *testing.T) {
	store := &countingSnapshotCache{inner: cache.NewMemoryCache[string, Snapshot]()}
	snapshot := Snapshot{
		Schema:    snapshotSchema,
		FetchedAt: time.Now(),
		Entries: map[string]map[string]Entry{
			"openai": {
				"gpt-test": {Intrinsic: Intrinsic{ContextWindow: 128_000, SupportsTools: true}},
			},
		},
	}
