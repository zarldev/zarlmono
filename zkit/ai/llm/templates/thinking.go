...
//	         thinking mode; the close marker has the pipe before `>`,
//	         literal — not a typo)
//
// Adding a new format is a one-line regex.
...
// whose wire format has no dedicated thinking channel (Ollama,
// llama.cpp, OpenAI-compatible). Anthropic exposes thinking as a
// distinct content block, so its client should skip this helper and
...
// matter. Empty inner blocks (e.g. Gemma-4 emits an empty thought
// channel when thinking is disabled) are dropped from the joined
// result.
