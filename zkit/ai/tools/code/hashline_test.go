...
func TestReadFileHLTool_ReportsGeneratedOwnership(t *testing.T) {
	t.Parallel()
	ws, root := mustWS(t)
	body := "// Code generated by goenums v0.4.9; DO NOT EDIT.\n//\n// goenums -f status_enum.go\npackage demo\n"
	if err := os.WriteFile(filepath.Join(root, "statuses_enums.go"), []byte(body), 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}

	res := execTyped(t, code.NewReadFileHLTool(ws), code.ReadFileHLArgs{Path: "statuses_enums.go"})
	if !res.Success {
		t.Fatalf("read failed: %s", res.Error)
	}
	got, _ := res.Data.(string)
	for _, want := range []string{"generated file", "goenums v0.4.9", "`status_enum.go`"} {
		if !strings.Contains(got, want) {
			t.Fatalf("generated notice missing %q:\n%s", want, got)
		}
	}
}

func TestReadFileHLTool_TruncationGivesContinuationArguments(t *testing.T) {
	t.Parallel()
	ws, root := mustWS(t)
	if err := os.WriteFile(filepath.Join(root, "long.txt"), []byte("one\ntwo\nthree\nfour\n"), 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}

	res := execTyped(t, code.NewReadFileHLTool(ws), code.ReadFileHLArgs{Path: "long.txt", Limit: 2})
	got, _ := res.Data.(string)
	if !strings.Contains(got, "continue with offset=2 limit=2") {
		t.Fatalf("truncation missing continuation arguments:\n%s", got)
	}
}
...
func TestEditFileHLTool_RejectsGeneratedFileWithoutWriting(t *testing.T) {
	t.Parallel()
	ws, root := mustWS(t)
	path := filepath.Join(root, "statuses_enums.go")
	original := "// Code generated by goenums v0.4.9; DO NOT EDIT.\n//\n// goenums -f status_enum.go\npackage demo\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}

	res := execTyped(t, code.NewEditFileHLTool(ws), code.EditFileHLArgs{
		Path: "statuses_enums.go",
		Edits: []code.HashlineEdit{{
			StartLine: 4,
			StartHash: testHashlineHash("package demo", 4),
			NewString: "package changed\n",
		}},
	})
	if res.Success {
		t.Fatal("expected generated-file edit refusal")
	}
	if res.Err == nil || res.Err.Kind != tools.Kinds.VALIDATION {
		t.Fatalf("error kind = %v, want VALIDATION", res.Err)
	}
	for _, want := range []string{"generated file", "`status_enum.go`"} {
		if !strings.Contains(res.Error, want) {
			t.Fatalf("generated refusal missing %q: %s", want, res.Error)
		}
	}
	if got := readFile(t, path); got != original {
		t.Fatalf("generated file changed on refusal: %q", got)
	}
}

func TestEditFileHLTool_DoesNotGuessGeneratedFromOrdinaryComment(t *testing.T) {
	t.Parallel()
	ws, root := mustWS(t)
	path := filepath.Join(root, "ordinary.go")
	if err := os.WriteFile(path, []byte("// generated values live elsewhere\npackage demo\n"), 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}
	...
	if !res.Success {
		t.Fatalf("ordinary edit failed: %s", res.Error)
	}
}
...
func TestEditFileHLTool_RejectsStaleHashWithoutWriting(t *testing.T) {
	...
	for _, want := range []string{
		"current anchors around the requested line",
		"1:" + testHashlineHash("alpha", 4) + "|alpha",
		"2:" + testHashlineHash("beta", 4) + "|beta",
	} {
		if !strings.Contains(res.Error, want) {
			t.Fatalf("stale recovery missing %q: %s", want, res.Error)
		}
	}
}

func TestEditFileHLTool_StaleLinePastEOFReturnsTailAnchors(t *testing.T) {
	t.Parallel()
	ws, root := mustWS(t)
	...
	res := execTyped(t, code.NewEditFileHLTool(ws), code.EditFileHLArgs{
		...
		Edits: []code.HashlineEdit{{
			StartLine: 99,
			StartHash: testHashlineHash("gone", 4),
	...
	for _, want := range []string{
		"file has 3 lines",
		"2:" + testHashlineHash("two", 4) + "|two",
		"3:" + testHashlineHash("three", 4) + "|three",
	} {
		if !strings.Contains(res.Error, want) {
			t.Fatalf("tail recovery missing %q: %s", want, res.Error)
		}
	}
}
