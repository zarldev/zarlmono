...
func TestLineRingBuffer_ZeroCapFallsBack(t *testing.T) {
	b := code.NewLineRingBuffer(0)
	for range 1025 {
		b.Append([]byte("x"))
	}
	count, total, dropped := b.Snapshot()
	if count != 1024 || total != 1025 || dropped != 1 {
		t.Errorf("Snapshot = %d/%d/%d, want 1024/1025/1", count, total, dropped)
	}
}
