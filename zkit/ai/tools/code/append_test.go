...
func TestWriteAppend_RejectsGeneratedFile(t *testing.T) {
	t.Parallel()
	ws := newTestWorkspace(t)
	path := filepath.Join(ws.Root(), "generated.go")
	original := "// Code generated by sqlc. DO NOT EDIT.\npackage generated\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
		t.Fatalf("seed: %v", err)
	}

	res := execTyped(t, code.NewWriteAppendTool(ws), code.WriteAppendArgs{Path: "generated.go", Content: "var X = 1\n"})
	if res.Success {
		t.Fatal("expected generated-file append refusal")
	}
	...
	if got, err := os.ReadFile(path); err != nil || string(got) != original {
		t.Fatalf("generated file changed: body=%q err=%v", got, err)
	}
}
