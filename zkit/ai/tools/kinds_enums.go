// Code generated by goenums v0.4.9; DO NOT EDIT.
//
// github.com/zarldev/goenums
//
// using the command:
// goenums -f kind_enum.go

package tools

import (
	"bytes"
	"database/sql/driver"
	"errors"
	"fmt"
	"iter"
	"math"

	"golang.org/x/exp/constraints"
)

// Kind is a type that represents a single enum value.
// It combines the core information about the enum constant and it's defined fields.
type Kind struct {
	kind
}

// kindsContainer is the container for all enum values.
// It is private and should not be used directly use the public methods on the Kind type.
type kindsContainer struct {
	UNKNOWN    Kind
	VALIDATION Kind
	NOTFOUND   Kind
	PERMISSION Kind
	TRANSIENT  Kind
	BUDGET     Kind
	FATAL      Kind
	STALE      Kind
}

// Kinds is a main entry point using the Kind type.
// It it a container for all enum values and provides a convenient way to access all enum values and perform
// operations, with convenience methods for common use cases.
var Kinds = kindsContainer{
	UNKNOWN: Kind{
		kind: unknown,
	},
	VALIDATION: Kind{
		kind: validation,
	},
	NOTFOUND: Kind{
		kind: notFound,
	},
	PERMISSION: Kind{
		kind: permission,
	},
	TRANSIENT: Kind{
		kind: transient,
	},
	BUDGET: Kind{
		kind: budget,
	},
	FATAL: Kind{
		kind: fatal,
	},
	STALE: Kind{
		kind: stale,
	},
}

// invalidKind is an invalid sentinel value for Kind
var invalidKind = Kind{
	kind: -1,
}

// allSlice returns a slice of all enum values.
// This method is useful for iterating over all enum values in a loop.
func (k kindsContainer) allSlice() []Kind {
	return []Kind{
		Kinds.VALIDATION,
		Kinds.NOTFOUND,
		Kinds.PERMISSION,
		Kinds.TRANSIENT,
		Kinds.BUDGET,
		Kinds.FATAL,
		Kinds.STALE,
	}
}

// All returns an iterator over all enum values.
// This method is useful for iterating over all enum values in a loop.
func (k kindsContainer) All() iter.Seq[Kind] {
	return func(yield func(Kind) bool) {
		for _, v := range k.allSlice() {
			if !yield(v) {
				return
			}
		}
	}
}

var ErrParseKind = errors.New("invalid input provided to parse to Kind")

// ParseKind parses the input value into an enum value.
// It returns the parsed enum value or an error if the input is invalid.
// It is a convenience function that can be used to parse enum values from
// various input types, such as strings, byte slices, or other enum types.
func ParseKind(input any) (Kind, error) {
	switch v := input.(type) {
	case Kind:
		return v, nil
	case string:
		if result := stringToKind(v); result != nil {
			return *result, nil
		}
	case fmt.Stringer:
		if result := stringToKind(v.String()); result != nil {
			return *result, nil
		}
	case []byte:
		if result := stringToKind(string(v)); result != nil {
			return *result, nil
		}
	case int:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case int8:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case int16:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case int32:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case int64:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case uint:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case uint8:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case uint16:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case uint32:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case uint64:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case float32:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	case float64:
		if result := numberToKind(v); result != nil {
			return *result, nil
		}
	default:
		return invalidKind, fmt.Errorf("invalid type %T", input)
	}
	return invalidKind, fmt.Errorf("%w: invalid value %v", ErrParseKind, input)
}

// kindsNameMap is a map of enum values to their Kind representation
// It is used to convert string representations of enum values into their Kind representation.
var kindsNameMap = map[string]Kind{
	"unknown":    Kinds.UNKNOWN,
	"validation": Kinds.VALIDATION,
	"not_found":  Kinds.NOTFOUND,
	"permission": Kinds.PERMISSION,
	"transient":  Kinds.TRANSIENT,
	"budget":     Kinds.BUDGET,
	"fatal":      Kinds.FATAL,
	"stale":      Kinds.STALE,
}

// stringToKind converts a string representation of an enum value into its Kind representation
// It returns a pointer to the Kind representation of the enum value if the string is valid
// Otherwise, it returns nil
func stringToKind(s string) *Kind {
	if t, ok := kindsNameMap[s]; ok {
		return &t
	}
	return nil
}

// numberToKind converts a numeric value to a Kind
// It returns a pointer to the Kind representation of the enum value if the numeric value is valid
// Otherwise, it returns nil
func numberToKind[T constraints.Integer | constraints.Float](num T) *Kind {
	f := float64(num)
	if math.Floor(f) != f {
		return nil
	}
	i := int(f)
	if i <= 0 || i > len(Kinds.allSlice()) {
		return nil
	}
	result := Kinds.allSlice()[i-1]
	if !result.IsValid() {
		return nil
	}
	return &result
}

// ExhaustiveKinds iterates over all enum values and calls the provided function for each value.
// This function is useful for performing operations on all valid enum values in a loop.
func ExhaustiveKinds(f func(Kind)) {
	for _, p := range Kinds.allSlice() {
		f(p)
	}
}

// validKinds is a map of enum values to their validity
var validKinds = map[Kind]bool{
	Kinds.UNKNOWN:    false,
	Kinds.VALIDATION: true,
	Kinds.NOTFOUND:   true,
	Kinds.PERMISSION: true,
	Kinds.TRANSIENT:  true,
	Kinds.BUDGET:     true,
	Kinds.FATAL:      true,
	Kinds.STALE:      true,
}

// IsValid checks whether the Kinds value is valid.
// A valid value is one that is defined in the original enum and not marked as invalid.
func (k Kind) IsValid() bool {
	return validKinds[k]
}

// MarshalJSON implements the json.Marshaler interface for Kind.
// It returns the JSON representation of the enum value as a byte slice.
func (k Kind) MarshalJSON() ([]byte, error) {
	return []byte("\"" + k.String() + "\""), nil
}

// UnmarshalJSON implements the json.Unmarshaler interface for Kind.
// It parses the JSON representation of the enum value from the byte slice.
// It returns an error if the input is not a valid JSON representation.
func (k *Kind) UnmarshalJSON(by []byte) error {
	by = bytes.Trim(bytes.Trim(by, "\""), "\"")
	newk, err := ParseKind(by)
	if err != nil {
		return err
	}
	*k = newk
	return nil
}

// MarshalText implements the encoding.TextMarshaler interface for Kind.
// It returns the string representation of the enum value as a byte slice
func (k Kind) MarshalText() ([]byte, error) {
	return []byte("\"" + k.String() + "\""), nil
}

// UnmarshalText implements the encoding.TextUnmarshaler interface for Kind.
// It parses the string representation of the enum value from the byte slice.
// It returns an error if the byte slice does not contain a valid enum value.
func (k *Kind) UnmarshalText(by []byte) error {
	newk, err := ParseKind(by)
	if err != nil {
		return err
	}
	*k = newk
	return nil
}

// Scan implements the database/sql.Scanner interface for Kind.
// It parses the string representation of the enum value from the database row.
// It returns an error if the row does not contain a valid enum value.
func (k *Kind) Scan(value any) error {
	newk, err := ParseKind(value)
	if err != nil {
		return err
	}
	*k = newk
	return nil
}

// Value implements the database/sql/driver.Valuer interface for Kind.
// It returns the string representation of the enum value.
func (k Kind) Value() (driver.Value, error) {
	return k.String(), nil
}

// MarshalBinary implements the encoding.BinaryMarshaler interface for Kind.
// It returns the binary representation of the enum value as a byte slice.
func (k Kind) MarshalBinary() ([]byte, error) {
	return []byte("\"" + k.String() + "\""), nil
}

// UnmarshalBinary implements the encoding.BinaryUnmarshaler interface for Kind.
// It parses the binary representation of the enum value from the byte slice.
// It returns an error if the byte slice does not contain a valid enum value.
func (k *Kind) UnmarshalBinary(by []byte) error {
	newk, err := ParseKind(by)
	if err != nil {
		return err
	}
	*k = newk
	return nil
}

// MarshalYAML implements the yaml.Marshaler interface for Kind.
// It returns the string representation of the enum value.
func (k Kind) MarshalYAML() ([]byte, error) {
	return []byte(k.String()), nil
}

// UnmarshalYAML implements the yaml.Unmarshaler interface for Planet.
// It parses the byte slice representation of the enum value and returns an error
// if the YAML byte slice does not contain a valid enum value.
func (k *Kind) UnmarshalYAML(by []byte) error {
	newk, err := ParseKind(by)
	if err != nil {
		return err
	}
	*k = newk
	return nil
}

// kindNames is a constant string slice containing all enum values cononical absolute names
const kindNames = "unknownvalidationnot_foundpermissiontransientbudgetfatalstale"

// kindNamesMap is a map of enum values to their canonical absolute
// name positions within the kindNames string slice
var kindNamesMap = map[Kind]string{
	Kinds.UNKNOWN:    kindNames[0:7],
	Kinds.VALIDATION: kindNames[7:17],
	Kinds.NOTFOUND:   kindNames[17:26],
	Kinds.PERMISSION: kindNames[26:36],
	Kinds.TRANSIENT:  kindNames[36:45],
	Kinds.BUDGET:     kindNames[45:51],
	Kinds.FATAL:      kindNames[51:56],
	Kinds.STALE:      kindNames[56:61],
}

// String implements the Stringer interface.
// It returns the canonical absolute name of the enum value.
func (k Kind) String() string {
	if str, ok := kindNamesMap[k]; ok {
		return str
	}
	return fmt.Sprintf("kind(%d)", k.kind)
}

// Compile-time check that all enum values are valid.
// This function is used to ensure that all enum values are defined and valid.
// It is called by the compiler to verify that the enum values are valid.
func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the goenums command to generate them again.
	// Does not identify newly added constant values unless order changes
	var x [8]struct{}
	_ = x[unknown]
	_ = x[validation-1]
	_ = x[notFound-2]
	_ = x[permission-3]
	_ = x[transient-4]
	_ = x[budget-5]
	_ = x[fatal-6]
	_ = x[stale-7]
}
