...
type schemaTool struct{ schema llm.Schema }

func (s schemaTool) Definition() tools.ToolSpec {
	return tools.ToolSpec{Name: "schema-test", Parameters: s.schema}
}

func (schemaTool) Execute(context.Context, tools.ToolCall) (*tools.ToolResult, error) {
	return &tools.ToolResult{Success: true}, nil
}

func validateJSONSchema(t *testing.T, schema llm.Schema, value any) error {
	t.Helper()
	guard := guardrails.NewSchemaGuardrail(tools.NewRegistry(schemaTool{schema: schema}))
	arguments, ok := value.(map[string]any)
	if !ok {
		arguments = map[string]any{"value": value}
		schema = llm.SchemaFromMap(map[string]any{
			"type":       "object",
			"properties": map[string]any{"value": schema.Map()},
		})
		guard = guardrails.NewSchemaGuardrail(tools.NewRegistry(schemaTool{schema: schema}))
	}
	return guard.Before(t.Context(), tools.ToolCall{
		ToolName:  "schema-test",
		Arguments: tools.ToolParameters(arguments),
	})
}
func TestValidateJSONSchema_Types(t *testing.T) {
	schema := map[string]any{"type": "string"}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), "hello"); err != nil {
		t.Errorf("string value: %v", err)
	}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), 42); err == nil {
...
func TestValidateJSONSchema_IntegerFromFloat(t *testing.T) {
	...
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), float64(42)); err != nil {
		t.Errorf("float64(42) against integer schema: %v", err)
	}
	// A fractional float64 should fail.
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), 42.5); err == nil {
...
func TestValidateJSONSchema_RequiredFields(t *testing.T) {
	...
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), good); err != nil {
		t.Errorf("good value: %v", err)
	}
	bad := map[string]any{}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), bad); err == nil {
...
func TestValidateJSONSchema_Enum(t *testing.T) {
	...
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), "a"); err != nil {
		t.Errorf("enum match: %v", err)
	}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), "d"); err == nil {
...
func TestValidateJSONSchema_RequiredStringSlice(t *testing.T) {
	...
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), map[string]any{"command": "ls"}); err != nil {
		t.Errorf("[]string required good: %v", err)
	}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), map[string]any{}); err == nil {
...
func TestValidateJSONSchema_NullPasses(t *testing.T) {
	...
	schema := map[string]any{"type": "string"}
	if err := validateJSONSchema(t, llm.SchemaFromMap(schema), nil); err != nil {
