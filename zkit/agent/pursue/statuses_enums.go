...
func ParseStatus(input any) (Status, error) {
	...
	case Status:
		return v, nil
	case string:
		if result := stringToStatus(v); result != nil {
			return *result, nil
		}
	case fmt.Stringer:
		if result := stringToStatus(v.String()); result != nil {
			return *result, nil
		}
	case []byte:
		if result := stringToStatus(string(v)); result != nil {
			return *result, nil
		}
	case int:
		if result := numberToStatus(v); result != nil {
			return *result, nil
		}
	case int8:
		if result := numberToStatus(v); result != nil {
...
var statusesNameMap = map[string]Status{
	...
	"gave_up":   Statuses.GAVEUP,
	"errored":   Statuses.ERRORED,
}

// stringToStatus converts a string representation of an enum value into its Status representation
// It returns a pointer to the Status representation of the enum value if the string is valid
...
var validStatuses = map[Status]bool{
	...
	Statuses.GAVEUP:    true,
	Statuses.ERRORED:   true,
}
...
const statusNames = "succeededgave_uperrored"
...
var statusNamesMap = map[Status]string{
	Statuses.SUCCEEDED: statusNames[0:9],
	Statuses.GAVEUP:    statusNames[9:16],
	Statuses.ERRORED:   statusNames[16:23],
}
