...
func TestMultiHandlerDispatchAndErrors(t *testing.T) {
	...
	err := derived.Handle(t.Context(), slog.NewRecord(time.Now(), slog.LevelInfo, "message", 0))
...
func TestMultiHandlerEmptyIsDisabled(t *testing.T) {
	t.Parallel()
	if zlog.NewMultiHandler().Enabled(t.Context(), slog.LevelInfo) {
		t.Fatal("empty multi-handler is enabled")
	}
}
